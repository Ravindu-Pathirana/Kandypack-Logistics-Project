...
# Pydantic schema for store creation
...
def create_store(store: StoreCreate, user_role: str):
    ...
    try:
        # Insert new store
        query = """
            INSERT INTO store (store_name, contact_number)
            VALUES (%s, %s)
        """
        cursor.execute(query, (store.store_name, store.contact_number))
        conn.commit()
        
        # Fetch the inserted store
        store_id = cursor.lastrowid
        cursor.execute("SELECT store_id, store_name, contact_number FROM store WHERE store_id = %s", (store_id,))
